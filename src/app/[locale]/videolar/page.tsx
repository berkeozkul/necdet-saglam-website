...
export async function generateMetadata() {
  ...
  return {
    title: `${t("videos")} | Prof. Dr. Necdet Sağlam`,
    ...
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      <section className="bg-primary py-20 text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10 text-center">
          <h1 className="font-heading text-4xl md:text-5xl font-bold mb-4">{nav("videos")}</h1>
          ...
          {!videos || videos.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-3xl shadow-sm border border-primary/5">
              ...
              <h2 className="text-2xl font-bold text-primary mb-2">
                {locale === 'tr' ? 'Henüz Video Yüklenmedi' : 'No Videos Uploaded Yet'}
              </h2>
              <p className="text-foreground/60">
                {locale === 'tr' ? 'Bu sayfaya yakında yeni videolar eklenecektir.' : 'New videos will be added to this page soon.'}
              </p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {videos.map((video) => {
                const videoTitle = locale === 'tr' ? video.title : video.title_en || video.title;
                return (
      ...
      <section className="py-16 bg-accent text-center">
        <div className="container mx-auto px-4">
          <h2 className="font-heading text-2xl md:text-3xl font-bold text-primary mb-6">
            {locale === 'tr' ? 'Daha Fazla Bilgi İçin' : 'For More Information'}
          </h2>
